  ...
  Page({
      ...
      for (var i = 0; i < category.length; i++) {
        ...
        for (var j = 0; j < item.length; j++) {
          if (structsMap.has(item[j].goodId)) {
            ...
            goodNum += item[j].commodityNum;
            goodPrice += item[j].commodityNum * parseInt(item[j].price)
          }
        }
      }
    ...
    setShowByCategory: function(category) {
      ...
      for (var i = 0; i < tempCategory.length; i++) {
        ...
        var item = tempCategory[i].good;
        for (var j = 0; j < item.length; j++) {
          if (item[j].commodityNum > 0 || item[j].commodityNum < 0) {
            item[j].isShow = true;
            isShow = true;
          } else {
            item[j].isShow = false;
